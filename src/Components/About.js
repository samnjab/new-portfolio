...
export default function About({mode, setMode}) {
    const scrollBy = (e) => {
        ...
        const maxDelta = e.target.scrollTopMax / 2
        console.log('scrolldelta', scrollDelta, 'scrollMax', maxDelta)
        const percentage = (scrollDelta / maxDelta) * -100
        console.log('delta percent', percentage)
        const nextPercentageUnconstrained = parseFloat(e.target.dataset.prevScrollPercent) + percentage
        console.log('unconstrained', nextPercentageUnconstrained)
        const nextPercentage = Math.max(Math.min(nextPercentageUnconstrained, 0), -100)
        target.dataset.prevScrollPercent = nextPercentage
        target.dataset.prevScroll = target.scrollTop
        target.animate({
            transform: `translate3d(0%, ${nextPercentage}%, 0%)`
        ...
        <div className='about'>
            <div className='body'>
                <div className='wrapper'>
                    ...
                    onScroll= {(e) => scrollBy(e)}
                    data-prev-scroll='0'
                    data-prev-scroll-percent='0'
                    >
